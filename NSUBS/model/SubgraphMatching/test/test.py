...
def check_correctness(binary_path, data_graph_path, query_folder_path, expected_results):
    query_graph_path_list = glob.glob(f'{query_folder_path}/*.graph')
    
    # Defining helper function to avoid redundancy
    def validate(exec_type, type_list):
        for current_type in type_list:
            for query_graph_path in query_graph_path_list:
                execution_args = generate_args(
                    binary_path, '-d', data_graph_path, '-q', query_graph_path, '-filter', current_type,
                    '-order', 'GQL', '-engine', 'LFTJ')
                ...
                if rc == 0:
                    ...
                    for line in std_output_list:
                        if '#Embeddings' in line:
                            ...
                            break
                    if embedding_num != expected_embedding_num:
                        print(f'{exec_type} {current_type} {query_graph_name} is wrong. Expected {expected_embedding_num}, Output {embedding_num}')
                        exit(-1)
                else:
                    print(f'{exec_type} {current_type} {query_graph_name} error.')
                    exit(-1)

            print(f'{exec_type} {current_type} passes the correctness check.')

    filter_type_list = ['LDF', 'NLF', 'GQL', 'TSO', 'CFL', 'DPiso']
    validate("Filter type", filter_type_list)
    print('All filter types pass the correctness check.')

    order_type_list = ['QSI', 'GQL', 'TSO', 'CFL', 'DPiso', 'RI', 'VF2PP']
    validate("Order type", order_type_list)
    print('All order types pass the correctness check.')

    engine_type_list = ['LFTJ', 'EXPLORE', 'GQL', 'QSI', 'DPiso', 'CECI']
    validate("Engine", engine_type_list)

if __name__ == '__main__':
    input_binary_path = sys.argv[1]
    if not os.path.isfile(input_binary_path):
        print(f'The binary {input_binary_path} does not exist.')
        exit(-1)
    ...
    dir_path = os.path.dirname(os.path.realpath(__file__))
    input_data_graph_path = f'{dir_path}/data_graph/HPRD.graph'
